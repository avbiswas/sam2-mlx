...
def apply_bidirectional_correction(
    ...
    backward_state = VideoTrackState(model, pixels, width, height)
    seed_obj = backward_state.object(init_event.obj_id)
    source = forward_state.object(init_event.obj_id)
    seed_obj.cond_memories = [m for m in source.cond_memories if int(m["frame_idx"]) == correction.frame_idx]
    seed_obj.low_by_frame[correction.frame_idx] = source.low_by_frame[correction.frame_idx]
    backward, backward_timings = backward_state.propagate(obj_ids, list(range(correction.frame_idx, -1, -1)), {})
...
def run_bidirectional_middle(model, pixels: np.ndarray, width: int, height: int, frame_idx: int | None = None, point: tuple[float, float] | None = None) -> tuple[np.ndarray, dict]:
    ...
    forward, forward_timings = run_one_direction(model, pixels, list(range(middle, pixels.shape[0])), init, width, height)
    backward, backward_timings = run_one_direction(model, pixels, list(range(middle, -1, -1)), init, width, height)
    masks = forward
